...
/*
    ...
    *spi_MOSI <---->  PA7(17)
    *
    *spi 极性为正（时钟平时为高），写入AD7190时，AD7190在SCLK上升沿锁存数据。
    *读取AD7190时，数据在SCLK下降沿之后置于DOUT线上，因此您的处理器应在SCLK上升沿锁存数据。
    *
    *您可以随时写入通信寄存器。唯一需要监控DOUT/RDY的时候是读取转换结果时。
    *
    *写序列和读序列可以是连续的。因此，写操作与读操作之间无需使SCLK和DIN变为高电平。
    *
    *尝试写入模式或配置寄存器以更改增益或通道等内容。建议您随后读取这些寄存器，确保数据正确加载到寄存器中。
    *
    *SCLK支持的最大频率为5 MHz。
    *
    *
