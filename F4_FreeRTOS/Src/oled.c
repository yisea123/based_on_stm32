...
void
oled_mark_dot (uint8_t x, uint8_t y)
{

}
void
oled_write_horizontal_line (uint8_t line_index, uint8_t start, uint8_t end)
{

}

void
oled_write_vertical_line (uint8_t vertical_index, uint8_t start, uint8_t end)
{
...
oled_init (void)
{
	i2c1_info = (i2c1_typedef *) malloc ( sizeof(i2c1_typedef) );
	i2c1_info->oled_address = 0x78;
...
void
oled_set_pos (uint8_t x, uint8_t y)
{
...
oled_write_cmd (uint8_t cmd)
{
	...
	data_buff[1] = cmd;
	portENTER_CRITICAL();
	ret = HAL_I2C_Master_Transmit ( &hi2c1, i2c1_info->oled_address, data_buff,
					2, 200 );
	portEXIT_CRITICAL();
	return ret;
}
