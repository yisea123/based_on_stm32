...
//
//void
//send_DHCP_DISCOVER (void); /*向DHCP服务器发送发现报文*/
//void
//send_DHCP_REQUEST (void); /*向DHCP服务器发送请求报文*/
//void
//send_DHCP_RELEASE_DECLINE (char msgtype); /*向DHCP服务器发送释放报文*/
//
//void
//reset_DHCP_time (void); /*初始化DHCP计时器*/
//void
//DHCP_timer_handler (void); /*DHCP计时器*/
//void
//check_DHCP_Timeout (void); /*检查是否超时*/
//uint8
//check_leasedIP (void); /*检查获取的IP是否冲突*/
//uint8
//parseDHCPMSG (uint16 length); /*从DHCP服务器接收消息并解析*/
//
///**
// *@brief		DHCP定时初始化
// *@param		无
...
//	//for(uint8 i=0; i<3; i++)
//	//Delay_ms(800);
//	sendto ( SOCK_DHCP, (uint8 *) pRIPMSG, sizeof(RIP_MSG), ip,
//					DHCP_SERVER_PORT );
//
//#ifdef DHCP_DEBUG
...
//	pRIPMSG->OPT[i++] = hostName;
//	//set the host name
//	sprintf ( (char*) host_name, (char*) "%.4s-%02X%02X%02X", DEVICE_ID,
//					SRC_MAC_ADDR[3], SRC_MAC_ADDR[4], SRC_MAC_ADDR[5] );
//	pRIPMSG->OPT[i++] = (uint8) strlen ( (char*) host_name );
//	strcpy ( (char*) &(pRIPMSG->OPT[i]), (char*) host_name );
...
//	sendto ( SOCK_DHCP, (const uint8 *) pRIPMSG, sizeof(RIP_MSG), ip,
//					DHCP_SERVER_PORT );
//#ifdef DHCP_DEBUG
...
// */
//void
//send_DHCP_RELEASE_DECLINE (char msgtype)
//{
//	uint16 i = 0;
//	uint8 ip[4];
//
...
//	uint8 opt_len = 0;
//
//	len = recvfrom ( SOCK_DHCP, (uint8 *) pRIPMSG, length, svr_addr,
//					&svr_port );
//
//#ifdef DHCP_DEBUG
...
//		return 0;
//	}
//	if (memcmp ( pRIPMSG->chaddr, SRC_MAC_ADDR, 6 ) != 0
//					|| pRIPMSG->xid != htonl ( DHCP_XID )) {
//#ifdef DHCP_DEBUG
...
//
//	if (sendto ( SOCK_DHCP, (const uint8*) "CHECK_IP_CONFLICT", 17, GET_SIP,
//					5000 )) {
//#ifdef DHCP_DEBUG
//		printf(" Conflict>\r\n");
//#endif
...
//					SRC_MAC_ADDR[1], SRC_MAC_ADDR[2], SRC_MAC_ADDR[3],
